...
def list_face_search_photo_items(
    ...
    *,
    ...
    offset: int = 0,
    limit: int = 20,
) -> tuple[FaceSearchSession | None, list[UserSearchPhotoItem], int]:
    try:
    ...
    if participant is None:
        items = list_legacy_face_search_photo_items(session, event=event, search_session=search_session)
        return search_session, paginate_user_search_photo_items(items, offset=offset, limit=limit), len(items)
...
def paginate_user_search_photo_items(
    items: list[UserSearchPhotoItem],
    *,
    offset: int,
    limit: int,
    ...
    for item in page:
        item.photo_url = safe_photo_preview_url(item.photo)
    return page
